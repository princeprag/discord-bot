 ...
 */
export const readSlashDirectory = async (
  Becca: BeccaInt,
  directory: string
): Promise<SlashInt[]> => {
  try {
    ...
    const files = await readdir(
      join(process.cwd() + "/prod/slash/" + directory),
      "utf-8"
    );
    for (const file of files) {
      const name = file.split(".")[0];
      const mod = await import(
        join(process.cwd() + `/prod/slash/${directory}/${file}`)
      );
      result.push(mod[name] as SlashInt);
    }
    return result;
  } catch (err) {
    beccaErrorHandler(Becca, "read slash directory util", err);
    return [];
  }
};
