...
export const validateChannelPerms = async (
  ...
  try {
    const manageServer = BeccaMember.permissionsIn(channel).has("MANAGE_GUILD");
    const manageRoles = BeccaMember.permissionsIn(channel).has("MANAGE_ROLES");
    const manageChannels =
      BeccaMember.permissionsIn(channel).has("MANAGE_CHANNELS");
    const kickMembers = BeccaMember.permissionsIn(channel).has("KICK_MEMBERS");
    const banMembers = BeccaMember.permissionsIn(channel).has("BAN_MEMBERS");
    const sendMessages =
      BeccaMember.permissionsIn(channel).has("SEND_MESSAGES");
    const manageMessages =
      BeccaMember.permissionsIn(channel).has("MANAGE_MESSAGES");
    const embedLinks = BeccaMember.permissionsIn(channel).has("EMBED_LINKS");
    const attachFiles = BeccaMember.permissionsIn(channel).has("ATTACH_FILES");
    const readMessageHistory = BeccaMember.permissionsIn(channel).has(
      "READ_MESSAGE_HISTORY"
    );
    const addReactions =
      BeccaMember.permissionsIn(channel).has("ADD_REACTIONS");
